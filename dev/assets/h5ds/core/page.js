import { basicTpl, asyncBasic, basicEvent } from '../templete/basicTpl'; //坐标，尺寸基础模块
...
import { removeDataPage, removeDataLayer, AppDataChange, setPageClass } from '../common/AppDataFun.js';
...
class Page {
    constructor(page) {
        ...
        // index; // 当前第几页
        this.className = 'page';
        this.page = page; // 直接编辑当前page 对象 app里面的page ，不是new Page() 对象
    }
    ...
    initSetBox() {
        ...
        let appSliderTypeTpls = appSliderTypeTpl({
            ...
            time: slider['time']
        }, '_page'); // _page 作为不同的ID使用

        appSliderTypeTpls = `<div class="set-slider">${appSliderTypeTpls}</div>`;

        // 如果page没有背景，初始化一个空的模版
        if (!style) {
            style = {}
            $setPageStyle.empty().html(imgTpl() + bgTpls + bgColorTpls + appSliderTypeTpls);
        } else {
            // 如果page有背景图
            if (style['background-image']) {
                ...
                $setPageStyle.empty().html(imgTpls + bgTpls + bgColorTpls + appSliderTypeTpls);
            } else {
                // 只有背景色
                $setPageStyle.empty().html(imgTpl() + bgTpls + bgColorTpls + appSliderTypeTpls);
            }
    ...
    initEvent() {
        ...
        $layerlist.off('click.dellayer').on('click.dellayer', '.dellayer', function (e) {
            ...
            var $li = $(this).parent();
            var index = $li.index();
            $('#pageView').find('.layer').eq(index).remove();
            $li.remove();

            // 删除 AppData.data.pages[xx].layers[index] 
            ...
            // 重新设置 z-index
            $('#pageView').find('.layer').each( function(ind, elem) {
        ...
        $layerlist.off('click.copylayer').on('click.copylayer', '.copylayer', function (e) {
            ...
            let obj = self.page.layers[index];
            // 拷贝一份
            obj = JSON.parse(JSON.stringify(obj));
            // 前面插入
            self.page.layers.splice(index, 0, obj);
        ...
        $layerlist.off('click.showlayer').on('click.showlayer', '.showlayer', function (e) {
            ...
            var $li = $(this).parent();
            var index = $li.index();
            var $layer = $('#pageView').find('.layer').eq(index);
            if ($layer.is(':hidden')) {
                $layer.show();
        ...
        //选择layer 通过 layer
        $('#pageView').off('click.layer contextmenu.layer').on('click.layer contextmenu.layer', '.layer', function (e) {
    ...
    newLayer(index) {
        ...
        let pageIndex = 0;
        try {
            pageIndex = $('#pagesList').find('.active').index();
        } catch(e) {
            pageIndex = undefined;
        }
        // active 是哪个，就实例化哪个
        let layer = this.page.layers[index];
    ...
    //初始化方法
    init() {
        $('#setPageName').html(this.page.name);
        this.initLayerList();
        this.initSetBox();
        this.initPageDom();
        this.initEvent();
