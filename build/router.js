var {
  upload,
  uploadBase64
} = require('./action/upload.js');
var {
  login,
  getUser,
  logout
} = require('./action/login.js');
var {
  register
} = require('./action/register.js');
var {
  authorize
} = require('./lib/filter');
var {
  limitReq
} = require('./lib/limitRequest');
var {
  getSysMp3
} = require('./action/sysMp3.js'); // mp3
var {
  getSysImgs,
  getSysImgTypes
} = require('./action/sysImgs.js'); // 系统图片
var {
  getSysSvgs,
  getSysSvgTypes
} = require('./action/sysSvgs.js'); // 系统svg
var {
  getSysTpls,
  getSysTplsTypes
} = require('./action/sysTpls.js'); // 系统模板
var {
  getUserImgs,
  delUserImgs
} = require('./action/userImgs.js'); // 用户图片
var {
  getUserTpls,
  delUserTpls,
  addUserTpls
} = require('./action/userTpls.js'); // 用户模板
var {
  saveData
} = require('./action/saveData.js'); // 保存app 
var {
  getUserApps,
  getUserApp,
  delApp,
  addData
} = require('./action/userApp.js'); // 
var {
  imgCode
} = require('./action/imgCode.js'); // 验证码

var {
  getH5
} = require('./action/getH5'); // SEO专用
...
function router(app, express) {
  ...
  app.get('/ui', function (req, res) {
    res.sendfile('./tpl/ui.html');
  }); // ui
  app.get('/login', function (req, res) {
    res.sendfile('./tpl/login.html');
  }); // 登录
  app.get('/register', function (req, res) {
    res.sendfile('./tpl/register.html');
  }); // 注册
  app.get('/help', function (req, res) {
    res.sendfile('./tpl/help.html');
  }); // 帮助
  app.get('/plus', function (req, res) {
    res.sendfile('./tpl/plus.html');
  }); // 插件
  app.get('/license', function (req, res) {
    res.sendfile('./tpl/license.html');
  }); // 插件

  app.get('/case', authorize, function (req, res) {
    res.sendfile('./tpl/case.html');
  }); // 案例
  app.get('/edit', authorize, function (req, res) {
    res.sendfile('./tpl/edit.html');
  }); // 编辑器

  app.get('*', function (req, res) {
    res.sendfile('./tpl/noFind.html');
  }); // 404
